...
test("formats a heatwave period as a readable french date range", () => {
  assert.equal(formatHeatwaveDateRange("2026-05-05", "2026-05-08"), "05 mai à 08 mai");
  assert.equal(formatHeatwaveDateRange("2026-05-30", "2026-06-02"), "30 mai à 02 juin");
});
...
test("keeps duration and average temperature details in the summary", () => {
  assert.equal(
    ...
    "Canicule 05 mai à 08 mai (4 jours, Tmax moyenne 32.5 degC)"
  );
});
